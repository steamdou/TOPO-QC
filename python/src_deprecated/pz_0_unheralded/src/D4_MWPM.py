...
from python.src_deprecated.pz_0_unheralded.src.tableaux import *
...
class D4_Code:
    ...
    def measure_e_anyons(self):
        ...
        for idx in range(self.Nx*self.Ny):
            ...
            if m==0:
                #modify the weight of m_anyons
                for i in self.cn_dict['bR_stored_edges'][idx]:
                    self.w1_arr[i] += np.log(2)/2
                for i in self.cn_dict['bL_stored_edges'][idx]:
                    self.w1_arr[i] += np.log(2)/2
                # measure e_anyon
