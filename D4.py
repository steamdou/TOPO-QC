...
def ILP_decode(V, E1_list, E2_list, T1_list, T2_list,
    ...
    m = gp.Model("ILP_decode_global_strings")
...
class D4_Code:
    def __init__(self, l, encode_x, cn_dict, V, E1_list, E2_list, Gamma1, Gamma2, w1_arr, w2_arr):
    ...
    def measure_e_anyons(self):
        ...
        for idx in range(self.Nx*self.Ny):
            ...
            if m==0:
                measurement_output = measure_A(idx,(self.Nx,self.Ny),self.SS,self.DD,self.RR,self.bL,self.bR,self.LZ,self.LX_vec,self.LX_sign,self.cn_dict)
    ...
    def flux_correction(self):
        x, e1, e2 = ILP_decode(self.V, self.E1_list, self.E2_list, self.T1, self.T2, self.Gamma1, self.Gamma2, self.w1_arr, self.w2_arr)
        if x is None:
    ...
    def correct_e_anyons(self):
        ...
        for kv in range(self.Nx*self.Ny):
            outcome = measure_A(kv,(self.Nx,self.Ny),self.SS,self.DD,self.RR,self.bL,self.bR,self.LZ,self.LX_vec,self.LX_sign,self.cn_dict)
            if outcome == 5:
                return 5
            else:
        ...
        for site in range(self.Nx*self.Ny):
            outcome = measure_A(site,(self.Nx,self.Ny),self.SS,self.DD,self.RR,self.bL,self.bR,self.LZ,self.LX_vec,self.LX_sign,self.cn_dict)
            if outcome != 0:
