...
def measure_A(kv,linear_size,SS,DD,RR,bL_state,bR_state,LZ_state,LX_vec,LX_sign,cn_dict,rng):     
    '''
    ...
    if np.size(is_generator)==1: # A_k is generator
        ...
        if np.size(s_ncm_ind)==0: # no non-commuting stabilizers 
            ...
            # generate random outcome using NumPy Generator
            if rng is None:
                raise ValueError("measure_A requires a numpy.random.Generator rng for probabilistic outcomes.")
            m_outcome = int(rng.integers(0, 2))
...
def correct_e_anyons(linear_size,bL_state,bR_state,SS,DD,RR,LZ_state,LX_vec,LX_sign,cn_dict,rng):
    '''
    ...
    for kv in range(Nx*Ny):
        a_syndrome[kv]=measure_A(kv,linear_size,SS,DD,RR,bL_state,bR_state,LZ_state,LX_vec,LX_sign,cn_dict,rng)
